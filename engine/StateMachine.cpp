...
void StateMachine::ExternalEvent(BYTE newState, const EventData *pData)
{
    ...
    if(newState == EVENT_IGNORED) {
#ifndef EXETERNAL_EVENT_NO_HEAP_DATA
        ...
        // Execute the state enigine, This function call will only return
...
void StateMachine::StateEngine(const StateMapRowEx* const pStateMapEx)
{
    ...
    //While events are being generated keep excuting states
    while(m_eventGenerated)
    {
        //Error check that the new state is valid before preceeding
        ASSERT_TRUE(m_newState < MAX_STATES);
